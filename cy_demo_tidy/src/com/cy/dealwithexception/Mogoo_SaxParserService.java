...
public class Mogoo_SaxParserService {
	...
	/*
	 * InputStream inputStream xml文件输入流 String keyNode 其name属性需作为映射表键的节点名 String
	 * keySubNode 需获取其name属性值的keyNode的子节点名
	 */
	public Mogoo_SaxParserService(InputStream inputStream, String keyNode,
			String keySubNode) {
		try {
	 ...
	 * 返回xml解析数据
	 */
	public HashMap<String, RestoreEntry> getDataList() {
		return mSaxXmlHandler.mDataList;
	}

	public class SaxXmlHandler extends DefaultHandler {
		private HashMap<String, RestoreEntry> mDataList = null;
		private RestoreEntry mSubNodeData = null;
		...
		public void startDocument() throws SAXException {
			mDataList = new HashMap<String, RestoreEntry>();
		}
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (localName.equals(mKeyNode)) {
				mSubNodeData = null;
				if (atts != null) {
					mSubNodeData = new RestoreEntry();
					mPreKeyNodeName = atts.getValue("name");
					mIsRestart = Boolean.parseBoolean(atts.getValue("restart"));
		...
		/*
		 * 接收元素的结尾的通知。 参数意义如下： uri ：元素的命名空间 localName ：元素的本地名称（不带前缀） name
		 * ：元素的限定名（带前缀）
		 */
		@Override
		public void endElement(String uri, String localName, String name) throws SAXException {
			Log.d(TAG, "endElement uri : " + uri + " localName : " + localName + " name :" + name);
			if (localName.equals(mKeyNode)) {
				if (mSubNodeData != null) {
					mSubNodeData.isRestart = mIsRestart;
	...
	class RestoreEntry extends ArrayList<String> {
