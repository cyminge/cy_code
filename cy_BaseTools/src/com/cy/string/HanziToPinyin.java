...
public class HanziToPinyin {
        ...
        synchronized (HanziToPinyin.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    if (DEBUG) {
                        Tracer.d(TAG, "Self validation. Result: " + doSelfValidation());
                    }
                    sInstance = new HanziToPinyin(true);
                    return sInstance;
                }
            }
            Tracer.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
    ...
    private static boolean doSelfValidation() {
        ...
        for (char c : UNIHANS) {
            ...
            if (cmp >= 0) {
                Tracer.d(TAG, "Internal error in Unihan table. " + "The last string \"" + lastString
