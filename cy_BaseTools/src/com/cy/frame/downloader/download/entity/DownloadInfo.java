...
public final class DownloadInfo extends DownloadRequest implements Cloneable {
    ...
    public DownloadInfo(DownloadRequest request) {
        ...
        boolean increaseType = GamesUpgradeManager.isIncreaseType(request.mPackageName);
        if (increaseType) {
            mGameSize = GamesUpgradeManager.getOneAppInfo(request.mPackageName).mPatchSize;
        } else {
            mGameSize = request.mGameSize;
        }

        if (increaseType) {
            mDownloadUrl = GamesUpgradeManager.getOneAppInfo(request.mPackageName).mPatchUrl;
            mRawDownloadUrl = request.mDownloadUrl;
        } else {
            mDownloadUrl = request.mDownloadUrl;
            mRawDownloadUrl = request.mDownloadUrl;
        }
        mGameId = request.mGameId;
        mIconUrl = request.mIconUrl;
        mGameName = request.mGameName;
        mPackageName = request.mPackageName;
        ...
        mAutoDownload = request.mAutoDownload;
    }
