...
public class GamesUpgradeManager {
	...
	private static final String ICON_URL = "img";
	...
	private static void checkAppInfo() {
		for (int i = 0; i < sAppInfo.size(); i++) {
			PackageInfo p = Utils.getPackageInfoByName(sAppInfo.get(i).packageName);
	...
	private static boolean checkAppInfo(ArrayList<UpgradeAppInfo> upList) {
		...
		for (int i = 0; i < sAppInfo.size(); i++) {
			for (int j = 0; j < upList.size(); j++) {
				if (sAppInfo.get(i).packageName.equals(upList.get(j).packageName)) {
					break;
				}
				if (j == upList.size() - 1) {
					result = true;
	...
	private static boolean isNeedAdd(UpgradeAppInfo upApp, String checkKey) {
		for (int i = 0; i < sAppInfo.size(); i++) {
			...
			if (upApp.packageName.equals(localAppInfo.packageName)) {
	...
	public static void updateAppInfoContent(String packageName) {
		try {
			init();
			for (int i = 0; i < sAppInfo.size(); i++) {
				if (sAppInfo.get(i).packageName.equals(packageName)) {
	...
	private static JSONObject getJsonFromInfo(UpgradeAppInfo appInfo) {
		try {
			...
			jsonItem.put(PACKAGE, appInfo.packageName);
			jsonItem.put(GAME_ID, appInfo.gameid);
			jsonItem.put(NEW_VERSIONCODE, appInfo.mNewVersionCode);
			jsonItem.put(NEW_VERSIONNAME, appInfo.mNewVersionName);
			jsonItem.put(SIZE, appInfo.size);
			jsonItem.put(DOWNLOAD_URL, appInfo.downUrl);
	...
	private static UpgradeAppInfo createUpgradeAppInfo(JSONObject jsonObject) {
		Gson gson = new Gson();
		UpgradeAppInfo upgradeAppInfo = gson.fromJson(jsonObject.toString(), UpgradeAppInfo.class);
		PackageManager pm = BaseApplication.getAppContext().getPackageManager();
		PackageInfo packageInfo = Utils.getPackageInfoByName(upgradeAppInfo.packageName);
		if (packageInfo != null) {
			upgradeAppInfo.mCurVersionName = packageInfo.versionName;
		}
	...
	public static class UpgradeAppInfo extends GameBean {
		...
		public boolean isEquals(UpgradeAppInfo info) {
			return info.mNewVersionCode == mNewVersionCode && info.mUpgradeInfo.equals(mUpgradeInfo)
					&& info.mIsIncreaseType == mIsIncreaseType && info.downUrl.equals(downUrl)
