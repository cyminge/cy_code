...
//            BgDialogActivity.show(BgDialogActivity.DIALOG_MOBILE_HINT);
    ...
    private static int checkSDCardSpace(long size) {
        ...
        try {
            ...
            if ((double) stat.getAvailableBlocks() * (double) blockSize > Constant.MIN_SPACE_REQUIRED
                    + (int) size) {
...
//        return GAME_HALL_PKG.equals(pkgName) || AMIGAME_PKG.equals(pkgName);
    ...
    public static void launchGame(String packageName) {
        // if (Constant.AMIGO_PLAY_PACKAGE_NAME.equals(packageName)) {
        // Recharge.startAmigoPlayRechargeActivity();
        // return;
        // }
        //
        // try {
        ...
        // GameActivityUtils.startActivity(context, targetIntent);
        // GameActionUtil.postGameAction(packageName, Constant.ACTION_OPEN);
        // StatisHelper.get().send(StatisValue.OPEN_GAME, packageName,
        // StatisSourceManager.getInstance().getCurSource());
        // } catch (Exception e) {
        // ToastUtils.showLimited(R.string.can_not_launch_game);
        // }
    }
